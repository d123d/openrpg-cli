...
class GameRNG:
    ...
    Set ``SRD_CLI_RNG_TRACE=1`` to log every draw to stderr for
    ...
    _trace_enabled: bool = field(
        default_factory=lambda: os.environ.get("SRD_CLI_RNG_TRACE", "").strip() == "1",
        repr=False,
    )
    ...
    def trace_dump(self) -> str:
        ...
        return "\n".join(self._trace) if self._trace else "(no RNG trace — set SRD_CLI_RNG_TRACE=1)"
