...
from openrpg_cli.domain.state import (
    ActorState,
    ClockState,
    EffectState,
    EncounterState,
...
from openrpg_cli.rules.effects import (
    Condition,
    DurationKind,
    Effect,
    Phase,
...
from openrpg_cli.domain.codecs import _plain
from openrpg_cli.rules.turns import (
    roll_initiative,
    end_turn,
    TurnOrder,
    TurnState,
...
from openrpg_cli.rules.actions import CoreAction
...
def handle_create(state: GameState, cmd: CreateGame, rng: DeterministicRNG) -> ReductionResult:
...
def handle_put(state: GameState, cmd: PutEntity, rng: DeterministicRNG) -> ReductionResult:
...
def handle_roll(state: GameState, cmd: RollDie, rng: DeterministicRNG) -> ReductionResult:
...
def handle_legacy(state: GameState, cmd: RecordLegacy, rng: DeterministicRNG) -> ReductionResult:
...
def handle_resolve(state: GameState, cmd: ResolveTest, rng: DeterministicRNG) -> ReductionResult:
    p = dict(cmd.payload)
    test = D20Test(
        TestKind(p["kind"]),
...
def handle_damage(state: GameState, cmd: ApplyDamage, rng: DeterministicRNG) -> ReductionResult:
    p = dict(cmd.payload)
...
def handle_heal(state: GameState, cmd: Heal, rng: DeterministicRNG) -> ReductionResult:
    p = dict(cmd.payload)
    index, actor = _actor(state, str(p["actor_id"]))
...
def handle_death_operation(state: GameState, cmd: Command, rng: DeterministicRNG) -> ReductionResult:
    p = dict(cmd.payload)
    index, actor = _actor(state, str(p["actor_id"]))
    vitality = _vitality_from(actor.data.get("vitality"))
...
def handle_effect(state: GameState, cmd: Command, rng: DeterministicRNG) -> ReductionResult:
    effects = tuple(
        Effect(
            e.id,
            e.source_id or "",
...
def handle_phase7(state: GameState, cmd: Command, rng: DeterministicRNG) -> ReductionResult:
    p = dict(cmd.payload)
...
def handle_character(state: GameState, cmd: Command, rng: DeterministicRNG) -> ReductionResult:
