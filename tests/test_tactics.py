from srd_cli.rules.tactics import *
def setup():
 s=SpatialState(("a","b"),(RelativePosition("a","b",RangeBand.NEAR),));return ActionBudget(),s
def test_grapple_shove_cost_and_failure():
 b,s=setup(); result=grapple("a","b",b,s,success=False)
...
def test_opportunity_cover_visibility_underwater():
 b,s=setup(); triggers=opportunity_triggers(s,"a",RangeBand.FAR,("b",))
