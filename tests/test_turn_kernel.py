...
def state():
 return GameState("g",actors=(ActorState("a",{"speed":30},team_id="x"),ActorState("b",{},team_id="y")),teams=(TeamState("x",actor_ids=("a",)),TeamState("y",actor_ids=("b",))))
def test_messages_roundtrip_and_replay():
 ...
 snapshots=[]
 for _ in range(2):
  st=state();rng=GameRNG(7)
  for cmd in cmds:
   result=reduce_command(st,cmd,rng);st,rng=result.state,result.rng
...
def test_spoof_rejects_atomically():
 start=reduce_command(state(),StartEncounter("s","g",{"encounter_id":"e","actor_ids":["a","b"]}),GameRNG(1))
 out=reduce_command(start.state,EndTurn("e1","g",{"encounter_id":"e","actor_id":"nope"}),start.rng)
 assert out.state==start.state and out.rng==start.rng and out.events[0].type_tag=="command_rejected"
